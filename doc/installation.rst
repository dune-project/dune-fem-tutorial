...
.. note:: This tutorial is based on the upcoming 2.11 version of Dune.
   ...
   obtained by explicitly specifying the desired version.
   Note that a few of the features described in this tutorial will not be
...
.. code-block:: bash
  ...
  CMAKE_FLAGS="-DCMAKE_PREFIX_PATH=PATHLIST" pip install dune-fem
...
* At least C++17 compatible C++ compiler (e.g. g++ 10 or later)
* python (3.9 or later)

* **Required** Dune modules (same version, latest is recommended)
...
* **Optional** Dune modules (same version, latest is recommended)
...
* **Issue with C++ compiler:**
  If the gnu compiler is used, version needs to be 10 or later. This can be checked in terminal with
